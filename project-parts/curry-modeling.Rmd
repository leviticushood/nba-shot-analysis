...
# Voting Ensemble (NB+DT+LR)   3(best per)0.594     0.198   0.648  0.761  0.439
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Clutch Model Performance Summary (n=151, 3-feature models, no retraining)
# Clutch Definition: 4th Qtr/OT, last 5 min, margin <= 5
# ─────────────────────────────────────────────────────────────────────────────
# Model                   Accuracy  Kappa   AUC    Sens   Spec
# ─────────────────────────────────────────────────────────────────────────────
# Logistic Regression     0.642     0.186   0.641  0.828  0.345
# Naive Bayes             0.629     0.201   0.640  0.731  0.466
# Decision Tree           0.656     0.176   0.593  0.914  0.241
# Voting Ensemble         0.649     0.209   0.645  0.817  0.379
# ─────────────────────────────────────────────────────────────────────────────
# Winner: Voting Ensemble -- best AUC (0.645) and Kappa (0.209)
# Note: all models show high sensitivity, low specificity in clutch context
#       suggesting difficulty identifying made shots under pressure

# Clutch Analysis Observations:
#  - General models trained on ~48% make rate, clutch subset has ~38% make rate
#    creating a class distribution mismatch that limits clutch prediction reliability
#  - qtr and sec_remaining lose predictive value in clutch context due to near-zero
#    variance (all clutch shots occur in 4th/OT with low seconds remaining)
#  - 3-feature models (dist, margin, three_pt) are better suited for clutch evaluation
#    as these features retain meaningful variance within clutch situations
#  - High sensitivity and low specificity across clutch models suggests over-prediction
#    of misses -- model cannot explain clutch makes from general shot profile alone
#  - n=151 clutch observations is insufficient for statistically significant claims
#    about clutch vs general performance differences (wide CIs, p > 0.05 vs NIR)
#  - Cannot claim Curry performs better or worse in clutch -- training distribution
#    and sample size limitations combat any such interpretation
#  - Future work: clutch-specific model would require multi-season data to achieve
#    sufficient n for reliable inference
